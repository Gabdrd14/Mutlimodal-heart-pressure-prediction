...
à partir de trois types de données :
  ...
  Branche 3 — Séries battement par battement (N_BB × 30) :
...
Nouveaux channels utilisés :
  ...
  Scalaires ECG :
    ...
    pr_mean_ms, pr_median_ms, pr_std_ms,
    qt_median_ms,
    qtc_mean_ms, qtc_median_ms, qtc_std_ms

  Scalaires SCG :
    pep_mean_ms, pep_median_ms, pep_std_ms,
    et_mean_ms, et_median_ms, et_std_ms,
    ivct_mean_ms, ivct_median_ms, ivct_std_ms,
    ivrt_mean_ms, ivrt_median_ms, ivrt_std_ms

  Battement par battement (ECG) :
    rr_bb_ms, pr_bb_ms, qt_bb_ms, qtc_bb_ms

  Battement par battement (SCG) :
...
###Branche 3 : séries battement par battement ###
...
def load_scalar(mat: dict, key: str) -> float:
    ...
    """
    On charge un scalaire depuis le mat, et on retourne NaN si absent.
...
def load_bb(mat: dict, key: str, target_len: int = BB_LEN) -> np.ndarray:
    ...
    """
    On charge une série battement par battement, et on remplace NaN par interpolation linéaire,
    et on resample à target_len points.
    Enfin, on retourne un array float32 de shape (target_len,).
    ...
    ### On remplace NaN par interpolation linéaire ###
    nans = np.isnan(arr)
    if nans.all():
        return np.zeros(target_len, dtype=np.float32)
    if nans.any():
    ...
    ### On resample vers target_len via interpolation ###
...
class RHCDataset(Dataset):
    ...
    On Retourne :
        ...
        x_bb     : (N_BB, BB_LEN)     float32  — battement par battement normalisés (global)
    ...
    def __init__(self, segments_folder: str, augment: bool = False,
        ...
        ### On filtre les fichiers invalides ###
        valid, skipped = [], []
        for f in self.files:
            try:
                mat = sio.loadmat(f)
        ...
        ### On normalise battement par battement (globale) ###
    ...
    def _compute_bb_stats(self):
        ...
        """
        On calcule mean/std globales de chaque canal BB sur tout le dataset.
        ...
        ### on reshape : (N_files * BB_LEN, N_BB) pour avoir les stats par canal ###
    ...
    def __getitem__(self, idx: int):
        ...
        ### x_bb : battement par battement, normalisation globale (N_BB, BB_LEN) ###
        bb_channels = []
...
class BeatByBeatEncoder(nn.Module):
    ...
    """
    CNN 1D léger sur les séries battement par battement (normalisées globalement).
...
class RHCDataModuleV2(L.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        ### Stats battement par battement (sur train uniquement) ###
        all_bb = []
        for f in train_files:
            mat = sio.loadmat(f)
            row = [load_bb(mat, k, target_len=BB_LEN) for k in BB_KEYS]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--segments_folder", default="processed/features_rhc_2")
    parser.add_argument("--output_dir",      default="checkpoints_v3")
