...
def safe_indices(wave_list):
    """Filtrer les NaN et convertir en int pour indexer un tableau numpy."""
    return np.array([int(x) for x in wave_list if not np.isnan(x)], dtype=int)
...
if __name__ == "__main__":
    ...
    for fname in os.listdir(INPUT_FOLDER):
        ...
        ### Détection automatique des ondes P, Q, R, S, T avec NeuroKit ###
        ...
        p_peaks = safe_indices(waves["ECG_P_Peaks"])
        q_peaks = safe_indices(waves["ECG_Q_Peaks"])
        s_peaks = safe_indices(waves["ECG_S_Peaks"])
        t_peaks = safe_indices(waves["ECG_T_Peaks"])
